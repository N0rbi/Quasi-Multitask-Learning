...
class Layer:
    ...
    def __init__(self, model, in_dim, output_dim, activation=dynet.tanh, mlp=0, mlp_activation=dynet.rectify):
        ...
        self.mlp = mlp
        if mlp:
            print('>>> use mlp with dim {} ({})<<<'.format(mlp, mlp_activation), file=sys.stderr)
            mlp_dim = mlp
    ...
    def __call__(self, x, soft_labels=False, temperature=None, dropout=0.0):
        if self.mlp:
            act = self.mlp_activation
            x_in = act(self.W_mlp * x + self.b_mlp)
        else:
            x_in = x
        # from params to expressions no longer needed in newer versions of dynet
        # W = dynet.parameter(self.W)

        if dropout:
            W = dynet.dropout(self.W, dropout)
        else:
            W = self.W

        logits = W*x_in + self.b
